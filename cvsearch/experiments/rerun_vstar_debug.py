...
from CVSearch import get_cvsearch_response, get_cvsearch_leaf_batch_response
...
def main():
    ...
    parser.add_argument("--leaf-batch", action="store_true", help="Use leaf-batch pipeline instead of recursive search.")
    parser.add_argument("--leaf-depth-min", type=int, default=1)
    parser.add_argument("--leaf-depth-max", type=int, default=3)
    ...
    if args.enable_evidence_memory:
        if args.leaf_batch:
            evidence_compiler = build_leaf_batch_compiler(args, model_path, search_model)
        else:
            evidence_compiler = build_evidence_compiler(args, model_path, search_model)
    else:
    ...
    with open(answers_file, "w") as out:
        for idx in tqdm(selected):
            ...
            if args.leaf_batch:
                response = get_cvsearch_leaf_batch_response(
                    sam_model=sam3,
                    ...
                    leaf_depth_range=(args.leaf_depth_min, args.leaf_depth_max),
                    **search_kwargs,
                )
            else:
...
def build_leaf_batch_compiler(args: argparse.Namespace, model_path: Path, search_model=None) -> EvidenceMemoryCompiler:
    device = args.evidence_device or args.vlm_device
    attention_provider = QwenFilteredAttentionProvider(
        QwenFilteredAttentionConfig(
            model_path=str(model_path),
    ...
    return EvidenceMemoryCompiler(
        window_builder=LeafBatchWindowBuilder(attention_provider=attention_provider),
        keeper=BatchScoreRankingKeeper(
            verifier=dino_verifier,
